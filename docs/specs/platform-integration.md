...
| 8 | Windows filesystems do not represent the POSIX executable bit — Stat never reports it | exec facts are recorded where POSIX generation runs; on Windows, gate and the generate/release freshness compare skip the exec bit (`ExecBitDecidable`) instead of false-alarming, while a manifest *written* on Windows still turns loudly stale on POSIX CI |
